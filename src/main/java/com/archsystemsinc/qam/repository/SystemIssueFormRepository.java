...
/**
 */
public interface SystemIssueFormRepository extends JpaRepository<SystemIssueForm, Long>, JpaSpecificationExecutor<SystemIssueForm>{
	
		//Queries to Find Months based on CSR Lists by From Date and To Date
		@Query("SELECT MONTHNAME(c.createdDate) as month, YEAR(c.createdDate) as year,macLookupId,jurisdictionId,SystemIssueFormId FROM SystemIssueForm c WHERE EXTRACT(YEAR_MONTH FROM c.createdDate) >= :fromMonthYear and EXTRACT(YEAR_MONTH FROM c.createdDate) <= :toMonthYear "
		...
		@Query("SELECT c FROM SystemIssueForm c WHERE EXTRACT(YEAR_MONTH FROM c.createdDate) >= :fromMonthYear and EXTRACT(YEAR_MONTH FROM c.createdDate) <= :toMonthYear "
				+ "and c.macLookupId in (:macLookupIdList) and c.jurisdictionId in (:jurisdictionList) and c.recordStatus = 1")
	    ...
	    @Modifying
		@Query("update SystemIssueForm c set c.recordStatus = :status, c.updateddDate = :updatedDate where c.userId = :userId and EXTRACT(YEAR_MONTH FROM c.createdDate) = :monthYear and c.macLookupId = :macLookupId and c.jurisdictionId = :jurisdictionId" )
